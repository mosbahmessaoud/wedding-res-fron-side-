// lib\screens\clan admin\custom_calendar_picker_view.dart
...
export 'custom_calendar_picker_view.dart' show DateStatus, DateAvailability;
...
void initState() {
  ...
  try {
    ...
    final validatedDates = await ApiService.getGroomsWithValidatedReservations(widget.clanId);
    final pendingDates = await ApiService.getGroomsWithPendingReservations(widget.clanId);
    final validatedNotBelong = await ApiService.getGroomsWithValidatedReservationsNotBelong(widget.clanId);
    final pendingNotBelong = await ApiService.getGroomsWithPendingReservationsNotBelong(widget.clanId);
    ...
    // Tag not-belong reservations
    final List<dynamic> allValidated = [
      ...validatedDates.map((r) => {...r, 'not_belong_to_clan': false}),
      ...validatedNotBelong.map((r) => {...r, 'not_belong_to_clan': true}),
    ];
    final List<dynamic> allPending = [
      ...pendingDates.map((r) => {...r, 'not_belong_to_clan': false}),
      ...pendingNotBelong.map((r) => {...r, 'not_belong_to_clan': true}),
    ];
    ...
    processReservations(allValidated, true);
    processReservations(allPending, false);

    Map<String, List<dynamic>> validatedByDate = {};
    for (var reservation in allValidated) {
      final date1 = reservation['date1']?.toString();
      final date2 = reservation['date2']?.toString();
      if (date1 != null) validatedByDate.putIfAbsent(date1, () => []).add(reservation);
      if (date2 != null && date2 != date1) validatedByDate.putIfAbsent(date2, () => []).add(reservation);
    }
    ...
    for (var reservation in allPending) {
      final date1 = reservation['date1']?.toString();
      final date2 = reservation['date2']?.toString();
      if (date1 != null) pendingByDate.putIfAbsent(date1, () => []).add(reservation);
      if (date2 != null && date2 != date1) pendingByDate.putIfAbsent(date2, () => []).add(reservation);
    }
...
DateAvailability _getDateAvailability(DateTime date) {
  ...
  if (_specialReservationDates.contains(key)) {
    return DateAvailability(
      ...
      maxCapacity: _maxGroomsPerDate,
      note: 'حجز خاص من العشيرة',
      specialReservation: _specialReservationsMap[key],
    );
  }
  
  // Show existing reservations regardless of year limit
  if (_dateAvailabilities.containsKey(key)) {
    return _dateAvailabilities[key]!;
  }
  
  // Check if date exceeds max years allowed (only blocks NEW bookings on empty dates)
  if (dateOnly.isAfter(_effectiveLastDate)) {
    return DateAvailability(
      date: date,
      status: DateStatus.disabled,
...
Widget _buildBeautifulReservationTile(Map<String, dynamic> reservation, bool isValidated) {
  ...
  final bool notBelongToClan = reservation['not_belong_to_clan'] == true;
  ...
  final badgeColor = notBelongToClan ? Colors.purple : primaryColor;

  return Container(
    margin: const EdgeInsets.only(bottom: 12),
    padding: const EdgeInsets.all(20),
    decoration: BoxDecoration(
      color: notBelongToClan ? Colors.purple.shade50 : Colors.white,
      borderRadius: BorderRadius.circular(16),
      border: Border.all(
              ...
              children: [
                if (reservation['allow_others'] == true)
                  Container(
                    ...
                    child: const Text('يسمح بالانضمام', style: TextStyle(fontSize: 10, color: Colors.white, fontWeight: FontWeight.bold)),
                  ),
                if (reservation['allow_others'] == false)
                  Container(
                    ...
                    child: const Text('لا يسمح بانضمام آخرين', style: TextStyle(fontSize: 10, color: Colors.white, fontWeight: FontWeight.bold)),
                  ),
                const SizedBox(height: 8),
                Container(
                  padding: const EdgeInsets.all(8),
                ...
                Row(
                  children: [
                    Container(
                      ...
                      child: Icon(Icons.date_range, size: 16, color: primaryColor),
                    ),
