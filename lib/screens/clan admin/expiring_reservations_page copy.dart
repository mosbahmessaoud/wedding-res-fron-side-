...
//   @override
//   State<ExpiringReservationsPage> createState() =>
//       _ExpiringReservationsPageState();
// }
...
//   // Normalize: remove spaces, dashes, leading zeros, then add country code
//   String cleaned = phone.replaceAll(RegExp(r'[\s\-()]'), '');
//   if (cleaned.startsWith('0')) {
//     cleaned = '213${cleaned.substring(1)}'; // Algeria +213 — change if needed
...
//   } else {
//     if (mounted) {
//       _showSnackBar('تعذّر فتح واتساب', Colors.red.shade400);
//     }
//   }
// }
...
//     try {
//       // Fetch only pending reservations (they have expired_date_if_pending)
//       final pending = await ApiService.getPendingReservations();

//       final now = DateTime.now();

//       final List<dynamic> ten = [];
//       final List<dynamic> twenty = [];
//       final List<dynamic> thirty = [];
...
//         // Only future / expiring soon — skip already-expired
//         if (diff < 0) continue;
...
//       // Sort each list: soonest expiry first
//       _sortByExpiry(ten);
...
//   Future<void> _validateReservation(
//       int reservationId, String groomName) async {
...
//       int reservationId, String groomName, String currentStatus) async {
//     final currentPayment = 0.0;
//     double requiredPayment = 0.0;
//     try {
//       requiredPayment = await ApiService.getRequiredPayment();
//     } catch (_) {}

//     final amount = await _showPaymentDialog(
//         groomName, currentPayment, requiredPayment, currentStatus);
...
//       String title, String content, Color color, IconData icon) async {
...
//   Future<double?> _showPaymentDialog(String groomName, double current,
//       double required, String currentStatus) async {
...
//         tabs: [
//           _buildTab(
//               'أقل من 10 أيام', _expiring10.length, Colors.red.shade500),
//           _buildTab('أقل من 20 يوم', _expiring20.length,
//               Colors.orange.shade500),
//           _buildTab('أقل من 30 يوم', _expiring30.length,
...
//   Widget _buildList(
//       List<dynamic> reservations, int threshold, bool isDark) {
...
//                   const SizedBox(height: 16),
//                   Text(
//                     'لا توجد حجوزات تنتهي خلال $threshold يوم',
//                     style: TextStyle(
//                         fontSize: 15,
//                         color: isDark
//                             ? Colors.grey.shade400
...
//         itemBuilder: (ctx, i) =>
//             _buildCard(reservations[i], isDark),
//       ),
//     );
//   }
...
//                 Text(
//                   'يوم',
//                   style: TextStyle(fontSize: 9, color: urgColor),
//                 ),
//               ],
//             ),
//           ),
...
//           children: [
//             _buildDetails(res, reservationId, groomId,
//                 '$firstName $lastName', paymentStatus, isDark),
//           ],
//         ),
//       ),
//     );
...
//           if (expStr != null)
//             _buildExpiryHighlight(expStr, isDark),
...
//           _detailRow(
//               'اسم الولي:',
//               res['guardian_name'] ?? 'غير محدد',
//               isDark),
//           _detailRow(
//               'اسم الأب:',
//               res['father_name'] ?? 'غير محدد',
//               isDark),
//           _detailRow(
//               'رقم الهاتف:',
//               res['phone_number'] ?? 'غير محدد',
//               isDark),
//           _detailRow(
//               'اليوم الأول:', _formatDate(res['date1']), isDark),
//           if (res['date2_bool'] == true && res['date2'] != null)
//             _detailRow(
//                 'اليوم الثاني:', _formatDate(res['date2']), isDark),
//           _detailRow(
//               'حالة الدفع:',
...
//           _detailRow(
//               'تاريخ الإنشاء:',
//               _formatDateTime(res['created_at']),
//               isDark),
//           if (expStr != null)
//             _detailRow(
//                 'ينتهي في:', _formatDateTime(expStr), isDark,
//                 valueColor: _urgencyColor(_daysRemaining(expStr))),

//           // const SizedBox(height: 16),
//           // _buildActionButtons(
//           //     reservationId, groomId, groomName, paymentStatus),
//           const SizedBox(height: 16),
//           _buildActionButtons(
//             reservationId,
//             groomId,
//             groomName,
//             paymentStatus,
//             res['phone_number'] ?? '',        // ADD
//             res['guardian_phone'] ?? '',      // ADD
//           ),
//         ],
//       ),
//     );
...
//                 ),
//                 Text(
//                   'تاريخ الانتهاء: ${_formatDate(expStr)}',
//                   style: TextStyle(
...
//   // Widget _buildActionButtons(int reservationId, int groomId,
//   //     String groomName, String paymentStatus) {
...
//   //       _actionBtn(
//   //         onPressed: () =>
//   //             _togglePayment(reservationId, groomName, paymentStatus),
...
//   //       _actionBtn(
//   //         onPressed: () =>
//   //             _validateReservation(reservationId, groomName),
...
//   String groomName,
//   String paymentStatus,
//   String groomPhone,      // ADD
//   String guardianPhone,   // ADD
// ) {
//   final isSmall = MediaQuery.of(context).size.width < 600;
//   return Wrap(
//     spacing: 8,
...
//       // ),
//       // _actionBtn(
//       //   onPressed: () => _validateReservation(reservationId, groomName),
//       //   icon: Icons.check_rounded,
//       //   label: 'تأكيد',
//       //   color: Colors.green.shade500,
//       //   isSmall: isSmall,
