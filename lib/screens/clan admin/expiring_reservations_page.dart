...
class _ExpiringReservationsPageState extends State<ExpiringReservationsPage>
  ...
  // ── Sort state ──────────────────────────────────────────────────────────────
  String _sortBy = 'created_at'; // default: soonest expiry
...
Widget _buildActionButtons(int reservationId, int groomId, String groomName,
  ...
  final res = _findReservationById(reservationId);
  final currentPayment = double.tryParse(res?['payment']?.toString() ?? '0') ?? 0.0;
  final status = res?['status'] ?? 'pending_validation';
  final isArchived = res != null ? _isArchived(res) : false;
  final currentDate = res?['date1'] ?? '';
      ...
      if (status == 'pending_validation' || status == 'validated')
        _actionBtn(
          onPressed: () async {
            ...
            _togglePayment(reservationId, groomName, paymentStatus, currentPayment, requiredPayment);
          },
...
Map<String, dynamic>? _findReservationById(int id) {
  for (final list in [_expiring10, _expiring20, _expiring30, _expiredReservations]) {
    for (final r in list) {
      if ((r['id'] ?? 0) == id) return r as Map<String, dynamic>;
    }
  }
  return null;
}

bool _isArchived(Map<String, dynamic> res) {
  final date2 = res['date2'];
  final date2Bool = res['date2_bool'] ?? false;
  final date1 = res['date1'];
  try {
    ...
    final dateStr = (date2Bool && date2 != null) ? date2 : date1;
...
Future<void> _togglePayment(int reservationId, String groomName,
    String currentPaymentStatus, double currentPayment, double requiredPayment) async {
  final amount = await _showPaymentDialog(groomName, currentPayment, requiredPayment, currentPaymentStatus);
  if (amount == null) return;
  try {
    setState(() => _isLoading = true);
    await ApiService.changePaymentStatus(reservationId, amount);
