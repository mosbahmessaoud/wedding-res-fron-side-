...
//       .toList();
//   final allArchived = allNotBelong
//       .where((r) => _isReservationArchived(r))
//       .toList();
//   final allActive = allNotBelong
//       .where((r) => !_isReservationArchived(r) && (r['status'] ?? '') != 'cancelled')
//       .toList();
...
//   // REPLACE with actual cancelled data from API:
//   final allCancelled = _belongCancelledOutsideReservations
//       .map((r) => Map<String, dynamic>.from({...r as Map, '_nb_status': 'cancelled'}))
//       .toList();

//   // Keep archived from active list only (not from cancelled)
//   final allArchived = allBelongOutside
//       .where((r) => _isReservationArchived(r))
//       .toList();
//   final allActive = allBelongOutside
//       .where((r) => !_isReservationArchived(r) && (r['status'] ?? '') != 'cancelled')
//       .toList();
//   final allActiveValidated = allValidated
//       .where((r) => !_isReservationArchived(r) && (r['status'] ?? '') != 'cancelled')
