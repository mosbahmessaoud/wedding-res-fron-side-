...
//     super.dispose();
//   }
// // Updated _loadClanSettings method to ensure settings are available
// Future<void> _loadClanSettings() async {
//   if (_selectedClan == null) return;
  
//   try {
//     print('Loading settings for clan: ${_selectedClan!['id']}');
    
//     // Load general system settings
//     final settings = await ApiService.getSettings();
//     print('System settings loaded: $settings');
    
//     // Load specific clan settings if needed
//     // Note: You might need to add this API call if clan-specific settings are stored separately
//     // final clanSettings = await ApiService.getClanSettings(_selectedClan!['id']);
    
//     setState(() {
//       // Update the max capacity from settings
//       _maxCapacityPerDate = settings['max_grooms_per_date'] ?? 3;
      
//       // If clan settings are not embedded in clan data, merge them here
//       // _selectedClan!['settings'] = clanSettings;
//     });
    
//     print('Updated max capacity per date: $_maxCapacityPerDate');
    
//     // Re-check two day availability after settings are loaded
...
//  // Updated _onClanSelected method to ensure settings are properly loaded
...
//     if (clan != null) {
//       print('Clan selected: ${clan['name']}');
//       print('Clan data: $clan');
      
//       // Load halls for this clan
//       _loadHallsForClan(clan['id']);
      
//       // Load clan settings for capacity and two-day rules
//       _loadClanSettings();
      
//       // Re-check two day availability if date is already selected
//       if (_date1Controller.text.isNotEmpty) {
//         _updateTwoDayAvailability();
//       }
//     } else {
...
// // Updated _updateTwoDayAvailability method with proper month checking
...
//     print('Selected clan: ${_selectedClan!['name']}');
    
//     // Get clan settings - this should be loaded when clan is selected
//     final settings = _selectedClan!['settings'] as Map<String, dynamic>?;
    
//     if (settings == null) {
//       print('No settings found for clan');
//       setState(() {
//         _canSelectTwoDays = false;
//         _date2Bool = false;
//       });
...
//     print('Clan settings: $settings');

//     // First check if clan allows two day reservations at all
//     final allowTwoDay = settings['allow_two_day_reservations'] == true;
//     print('Clan allows two day reservations: $allowTwoDay');
...
//     final twoDateMonthsString = settings['allowed_months_two_day']?.toString() ?? '';
//     print('Allowed months string: "$twoDateMonthsString"');
    
//     if (twoDateMonthsString.isEmpty) {
...
//     // Parse allowed months (comma-separated string like "1,2,12")
//     final allowedMonths = <int>{};
//     for (String monthStr in twoDateMonthsString.split(',')) {
//       final month = int.tryParse(monthStr.trim());
...
//     final isMonthAllowed = allowedMonths.contains(selectedMonth);
//     print('Is month $selectedMonth allowed for two days: $isMonthAllowed');
    
//     setState(() {
//       _canSelectTwoDays = isMonthAllowed;
...
//         ScaffoldMessenger.of(context).showSnackBar(
//           SnackBar(
//             content: Text('يمكن حجز يومين متتاليين في ${_getMonthName(selectedMonth)}'),
...
//         ScaffoldMessenger.of(context).showSnackBar(
//           SnackBar(
//             content: Text('حجز يوم واحد فقط متاح في ${_getMonthName(selectedMonth)}'),
...
//   return month >= 1 && month <= 12 ? monthNames[month] : 'شهر غير صحيح';
// }
//   @override
...
// // Updated date selection handler to ensure proper checking
// void _handleAvailableDate(DateTime selectedDate, TextEditingController controller) {
//   setState(() {
//     controller.text = selectedDate.toLocal().toString().split(' ')[0];
//   });
  
//   // Check two-day availability after setting the date
//   if (controller == _date1Controller) {
//     _updateTwoDayAvailability();
//   }
...
// // Updated fallback date picker to also check two-day availability
